...
#Region "Load Initial Values"
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            ...
            ObjCat.Catalogo(cmbComplementoFactura, "select id, nombre from tblComplementosFactura where id in (1) order by id", 0)
            ObjCat.Catalogo(cmbBodegaDestinoCoppel, "select codigo, concat(codigo,' - ',nombre) from tblBodegaEntCoppel", 0)
            Try
                ObjCat.Catalogo(cmbUUID, "select top 4800 uuid, convert(varchar(10), fecha_factura, 103) + ' | ' + isnull(serie,'') + convert(varchar(10), folio) + ' - ' + isnull(uuid,'') as folio from tblCFD where isnull(uuid,'')<>'' and (estatus=1 or estatus=3) and serie='A' and clienteid='" & cmbCliente.SelectedValue.ToString & "' order by fecha_factura desc", 0)
        ...
        Dim obt As New AddendaCoppelDAO
        Dim addenda As New AddendaCoppelDTO
    ...
    Private Sub CargaCliente(ByVal ClienteId As Long)
        ...
        Try
            ...
            If rs.Read() Then
                ...
                ObjCat.Catalogo(cmbUUID, "select top 4800 uuid, convert(varchar(10), fecha_factura, 103) + ' | ' + isnull(serie,'') + convert(varchar(10), folio) + ' - ' + isnull(uuid,'') as folio from tblCFD where isnull(uuid,'')<>'' and (estatus=1 or estatus=3) and serie='A' and clienteid='" & ClienteId.ToString & "' order by fecha_factura desc", 0)
                ObjCat = Nothing
...
#Region "Create Invoice"
    ...
    Private Function Descomprimir(ByVal data5 As Byte())
        ...
        If File.Exists(Path & archivo) Then
            System.IO.File.Copy(Path & archivo, Path & "ng_" & serie.ToString & folio.ToString & "_timbrado.xml", False)
        End If
        ...
        Try
            ...
            While rs.Read
                ...
                If CBool(rs("exento")) = False Then
                    ...
                    Importe = Format(CDbl(rs("iva")), "#0.00")
...
#End Region
    ...
    Private Sub CrearNodoReceptor(ByVal Nodo As XmlNode, ByVal nombre As String, ByVal rfc As String, ByVal UsoCFDI As String)
        Try
            ...
            Receptor.SetAttribute("Nombre", nombre.Replace("`", "'"))
...
#Region "Addenda Coopel"
    ...
    Private Sub AgregaAddendaCoopelRopa()
        ...
        addendaDTO.FleteCaja = itemsList.MasterTableView.Items.Count
    ...
    Private Sub btnAddenda_Click(sender As Object, e As EventArgs) Handles btnAddenda.Click
        ...
        addendaDTO.Cfdid = 233505 '232488
        addendaDTO.FechaPromesaEntrega = calFechaPromesaEntrega.SelectedDate
        addendaDTO.FechaOrdenCompra = calFechaOrdenCompraDS.SelectedDate
        addendaDTO.FleteCaja = itemsList.MasterTableView.Items.Count
        addendaDTO.BodegaDestino = cmbBodegaDestinoCoppel.SelectedValue
