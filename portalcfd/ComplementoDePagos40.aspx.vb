...
Partial Public Class ComplementoDePagos40
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            ...
            ObjCat.Catalogo(cmbCliente, "EXEC pCatalogos @cmd=2 ", 0)
            ...
            ObjCat.Catalogo(cmbCuentaConcentradora, "select id, nombre  from tblCuentasConcentradoras where isnull(borradorbit,0)=0 order by nombre", 0)
            ObjCat = Nothing
            ...
            fha_ini.SelectedDate = Now.AddDays(-7)
    ...
    Private Sub CargaClientePorCuenta(ByVal ClienteId As Long)
        Dim conn As New SqlConnection(System.Configuration.ConfigurationManager.ConnectionStrings("conn").ConnectionString)
        Dim cmd As New SqlCommand("EXEC pMisClientes @cmd=9, @cuenta_concentradoraid='" & ClienteId.ToString & "'", conn)
    ...
    Function ObtenerItems() As DataSet
        ...
        Dim cmd As New SqlDataAdapter("EXEC pComplementoDePago @cmd=1, @clienteid='" & cmbCliente.SelectedValue & "', @tipoid='1', @fhaini='" & fha_ini.SelectedDate.Value.ToShortDateString & "', @fhafin='" & fha_fin.SelectedDate.Value.ToShortDateString & "', @cuenta_concentradoraid='" & cmbCuentaConcentradora.SelectedValue & "'", conn)
    ...
    Protected Sub GetCFD()
        ...
        Dim conn As New SqlConnection(System.Configuration.ConfigurationManager.ConnectionStrings("conn").ConnectionString)
        Dim cmd As New SqlCommand("EXEC pCFD @cmd=51, @clienteid='" & cmbCliente.SelectedValue & "', @cuenta_concentradoraid='" & cmbCuentaConcentradora.SelectedValue & "'", conn)
    ...
    Protected Sub GetPAGOCFD()
        ...
        If fecha.SelectedDate Is Nothing Then
            sql = "EXEC pComplementoDePago @cmd=13, @clienteid='" & cmbCliente.SelectedValue & "', @cuenta_concentradoraid='" & cmbCuentaConcentradora.SelectedValue & "', @formapagoid='" & cmbFormaPago.SelectedValue & "', @numoperacion='" & txtNumOperacion.Text & "', @ctaordenante='" & cmbCtaOrdenante.SelectedItem.Text & "', @rfcemisorctaord='" & txtRFCCtaOrdenante.Text & "', @ctabeneficiaria='" & cmbCtaBeneficiario.SelectedItem.Text & "', @rfcemisorctabeneficiaria='" & txtRfcBeneficiario.Text & "', @nomBancoOrdext='" & txtBancoExtr.Text & "'"

        Else
            fechaP = fecha.SelectedDate.Value.ToShortDateString & " " & "12:00:00"
            sql = "EXEC pComplementoDePago @cmd=13, @clienteid='" & cmbCliente.SelectedValue & "', @cuenta_concentradoraid='" & cmbCuentaConcentradora.SelectedValue & "', @formapagoid='" & cmbFormaPago.SelectedValue & "', @numoperacion='" & txtNumOperacion.Text & "', @ctaordenante='" & cmbCtaOrdenante.SelectedItem.Text & "', @rfcemisorctaord='" & txtRFCCtaOrdenante.Text & "', @ctabeneficiaria='" & cmbCtaBeneficiario.SelectedItem.Text & "', @rfcemisorctabeneficiaria='" & txtRfcBeneficiario.Text & "', @nomBancoOrdext='" & txtBancoExtr.Text & "', @fecha_pago='" & fechaP.ToString & "'"
        End If
    ...
    Private Sub cmbFormaPago_SelectedIndexChanged(ByVal sender As Object, ByVal e As EventArgs) Handles cmbFormaPago.SelectedIndexChanged
        If cmbFormaPago.SelectedValue.Length > 0 Then
            If cmbFormaPago.SelectedValue.ToString = "15" Then
                'txtuuidrelacionado.Visible = True
                'lbluuidrelacionado.Visible = True
            End If
    ...
    Private Sub cmbCuentaConcentradora_SelectedIndexChanged(ByVal sender As Object, ByVal e As EventArgs) Handles cmbCuentaConcentradora.SelectedIndexChanged
        Call CargaClientePorCuenta(cmbCuentaConcentradora.SelectedValue)
        Call DisplayItems()
        panelItemsRegistration.Visible = True
        itemsList.Visible = True
    End Sub
    ...
    Private Sub ConfiguraReceptor()
        ...
        Dim cmdR As New SqlCommand("exec pCFD @cmd=52, @clienteId='" & cmbCliente.SelectedValue.ToString & "', @cuenta_concentradoraid='" & cmbCuentaConcentradora.SelectedValue & "'", connR)
        Try
    ...
    '    Pago.SetAttribute("Monto", Format(total / tipocambio, "#0.00"))
    ...
    '            '    DocumentoRelacionado.SetAttribute("Serie", rs("serie"))
    '            'End If
    '            'If ("folio").ToString.Length > 0 Then
    '            '    DocumentoRelacionado.SetAttribute("Folio", rs("folio"))
    '            'End If

    '            'If Not rs("moneda").ToString = "MXN" Then
    ...
    Private Sub CrearNodoPagos(ByVal Nodo As XmlNode)
        ...
        Dim ObjComp As New DataControl
        ObjComp.RunSQLQuery("exec pComplementoDePago @cmd=12, @formapagoid='" & cmbFormaPago.SelectedValue & "', @monedaid='" & cmbMoneda.SelectedValue & "', @tipocambio='" & tipocambio.ToString & "', @numoperacion='" & txtNumOperacion.Text & "', @fecha_pago='" & fechaP.ToString & "', @ctaordenante='" & cmbCtaOrdenante.SelectedItem.Text & "', @rfcemisorctaord='" & txtRFCCtaOrdenante.Text & "', @ctabeneficiaria='" & cmbCtaBeneficiario.SelectedItem.Text & "', @rfcemisorctabeneficiaria='" & txtRfcBeneficiario.Text & "', @nomBancoOrdext='" & txtBancoExtr.Text & "', @clienteid='" & cmbCliente.SelectedValue.ToString & "', @pagoid='" & Session("PAGOCFD").ToString & "'")
        ObjComp = Nothing
            ...
            While rs.Read
                ...
                If ds.Tables(0).Rows.Count > 0 Then
                    For Each row As DataRow In ds.Tables(0).Rows
                        ...
                        TotalTrasladosBaseIVA16 = TotalTrasladosBaseIVA16 + row("importepagado")

                        TotalBaseDr = TotalBaseDr + row("importepagado")
