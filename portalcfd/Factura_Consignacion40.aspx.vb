...
Public Class Factura_Consignacion40
    ...
    Dim dsItemsList As New DataSet()
...
#Region "Load Initial Values"
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            ...
            ObjCat.Catalogo(cmbBodegaDestinoCoppel, "select codigo, concat(codigo,' - ',nombre) from tblBodegaEntCoppel", 0)
            ObjCat = Nothing
    ...
    Private Sub CargaCFD()
        ...
        Try
            ...
            If rs.Read() Then
                ...
                If CBool(rs("descuentoAplicado")) Then
                    chkDescuentoAplicado.Checked = True
                    chkDescuentoAplicado.Enabled = False
                End If
                If CInt(rs("tipo_addenda")) <> 0 Then
                    cmbComplementoFactura.SelectedValue = rs("tipo_addenda")
                    cmbComplementoFactura.Enabled = False
                    Call viewComplementoFactura()
                End If
            End If
...
#Region "Add Invoice Items"
    ...
    Protected Sub InsertItem(ByVal id As Integer, ByVal item As GridItem)
        ...
        '
        ' Agrega Informacion complemento Comercio Exterior
        Dim txtNoIdentificacion As System.Web.UI.WebControls.TextBox = DirectCast(item.FindControl("txtNoIdentificacion"), System.Web.UI.WebControls.TextBox)
        Dim txtFraccArancelaria As System.Web.UI.WebControls.TextBox = DirectCast(item.FindControl("txtFraccArancelaria"), System.Web.UI.WebControls.TextBox)
        Dim txtCantAduana As RadNumericTextBox = DirectCast(item.FindControl("txtCantAduana"), RadNumericTextBox)
        Dim cmbUnidadAduana As System.Web.UI.WebControls.DropDownList = DirectCast(item.FindControl("cmbUnidadAduana"), System.Web.UI.WebControls.DropDownList)
        Dim txtUnitarioAduana As RadNumericTextBox = DirectCast(item.FindControl("txtUnitarioAduana"), RadNumericTextBox)
        ...
        Dim cantidad_aduana As Decimal = 0
        Dim unitario_aduana As Decimal = 0
        Dim valor_dolares As Decimal = 0
        ...
        Try
            cantidad_aduana = Convert.ToDecimal(txtCantAduana.Text)
        Catch ex As Exception
            cantidad_aduana = 0
        End Try
        Try
            unitario_aduana = Convert.ToDecimal(txtUnitarioAduana.Text)
        Catch ex As Exception
            unitario_aduana = 0
        End Try
        Try
            valor_dolares = Convert.ToDecimal(txtValorDolares.Text)
        Catch ex As Exception
            valor_dolares = 0
        End Try
        '
        ...
        descuento = (Convert.ToDecimal(porcentaje_descuento) / 100) * (Convert.ToDecimal(txtQuantity.Text) * Convert.ToDecimal(txtUnitaryPrice.Text))
        ...
        objdata.RunSQLQuery("EXEC pCFD @cmd=2, @cfdid='" & Session("CFD").ToString & "', @codigo='" & lblCodigo.Text & "', @descripcion='" & txtDescripcion.Text & "', @cantidad='" & cantidad.ToString & "', @unidad='" & lblUnidad.Text & "', @precio='" & txtUnitaryPrice.Text & "', @inventarioid='" & id.ToString & "', @descuentobit='0', @importe_descuento='" & descuento.ToString & "', @productoid='" & productoid.ToString & "', @importe_descuento='" & descuento.ToString & "', @talla='" & talla.ToString & "', @cajas='" & cajas.ToString & "', @piezasporcaja='" & piezasporcaja.ToString & "', @numero_identificacion='" & txtNoIdentificacion.Text.ToString & "', @fraccion_arancelaria='" & txtFraccArancelaria.Text.ToString & "', @cantidad_aduana='" & cantidad_aduana.ToString & "', @unidad_aduana='" & cmbUnidadAduana.SelectedValue.ToString & "', @unitario_aduana='" & unitario_aduana.ToString & "', @valor_dolares='" & valor_dolares.ToString & "'")
    ...
    Protected Sub itemsList_NeedDataSource(ByVal source As Object, ByVal e As Telerik.Web.UI.GridNeedDataSourceEventArgs) Handles itemsList.NeedDataSource
        itemsList_NeedData(canBind:=False)
    End Sub

    Private Sub itemsList_NeedData(Optional canBind As Boolean = True)
        Dim conn As New SqlConnection(System.Configuration.ConfigurationManager.ConnectionStrings("conn").ConnectionString)
    ...
    Protected Sub itemsList_ItemDataBound(ByVal sender As Object, ByVal e As Telerik.Web.UI.GridItemEventArgs) Handles itemsList.ItemDataBound
        ...
        If e.Item.ItemType = Telerik.Web.UI.GridItemType.Footer Then
            If itemsList.Items.Count > 0 Then
                ...
                For Each item As GridDataItem In itemsList.Items
                    totalCantidad += Convert.ToInt32(item("cantidad").Text)
                Next
                e.Item.Cells(4).Text = "Piezas:"
                e.Item.Cells(4).HorizontalAlign = HorizontalAlign.Right
                e.Item.Cells(4).Font.Bold = True

                e.Item.Cells(5).Text = totalCantidad.ToString()
...
#Region "Create Invoice"
    ...
    Private Sub CrearAtributosComprobante(ByVal Nodo As XmlElement, ByVal TipoDeComprobante As String)
        ...
        If cmbComplementoFactura.SelectedValue = 7 Then
            Nodo.SetAttribute("xsi:schemaLocation", "http://www.sat.gob.mx/cfd/4 http://www.sat.gob.mx/sitio_internet/cfd/4/cfdv40.xsd http://www.sat.gob.mx/ComercioExterior11 http://www.sat.gob.mx/sitio_internet/cfd/ComercioExterior11/ComercioExterior11.xsd")
        Else
    ...
    Private Sub CrearNodoReceptor(ByVal Nodo As XmlNode, ByVal nombre As String, ByVal rfc As String, ByVal UsoCFDI As String, ByVal DomicilioFiscalReceptor As String, ByVal ResidenciaFiscal As String, ByVal NumRegIdTrib As String, ByVal RegimenFiscalReceptor As String)
        ...
        Receptor.SetAttribute("Nombre", nombre.ToUpper.Replace("`", "'"))
    ...
    Private Sub ConfiguraReceptor()
        ...
        Try
            ...
            If rs.Read Then
                CrearNodoReceptor(Comprobante, rs("denominacion_razon_social"), rs("fac_rfc"), cmbUsoCFD.SelectedValue, rs("fac_cp"), rs("clave_pais"), rs("identidad_tributaria"), rs("regimenfiscalid"))
...
#End Region
    ...
    Sub txtPrecio_TextChanged(sender As Object, e As EventArgs)
        If Not chkDescuentoAplicado.Checked Then
        ...
        Call itemsList_NeedData()
        Call CargaTotales()
    End Sub

    Sub ActulizaAtributoPartida_PorNombreDelCampo(sender As Object, e As EventArgs)
        Dim txt = DirectCast(sender, RadNumericTextBox)
        Dim nombreCampo As String = txt.ID
        Dim cell = DirectCast(txt.Parent, GridTableCell)
        Dim item As GridDataItem = cell.Item
        Dim index As Integer = item.ItemIndex
        For Each dataItem As Telerik.Web.UI.GridDataItem In itemsList.MasterTableView.Items
            ...
            If dataItem.ItemIndex = index Then
                Dim partidaDAO As New CFDPartidasDAO(id)
                Select Case nombreCampo
                    Case "txtTalla"
                        partidaDAO.ActulizaTalla(txt.Text)
                    Case "txtCajas"
                        partidaDAO.ActulizaCajas(txt.Text)
                    Case "txtPiezasPorCaja"
                        partidaDAO.ActulizaPiezasPorCaja(txt.Text)
                End Select
                Exit For
            End If
        Next
    ...
    Private Sub cmbComplementoFactura_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cmbComplementoFactura.SelectedIndexChanged
        Call viewComplementoFactura()
    End Sub
    Private Sub viewComplementoFactura()
        Select Case cmbComplementoFactura.SelectedValue
            Case 1
                panelComplementoDetallista.Visible = True
            Case 6
                panelAddendaCoppel.Visible = True
                itemsList.MasterTableView.GetColumn("talla").Visible = True
                itemsList.MasterTableView.GetColumn("cajas").Visible = True
                itemsList.MasterTableView.GetColumn("piezasporcaja").Visible = True
            Case 7
            ...
            Case Else
                ...
                panelComercioExterior.Visible = False
        End Select
        Call DisplayItems()
    End Sub
...
#Region "Addenda Coopel"
    Private Sub AgregaAddendaCoopelRopa()
        ...
        addendaDTO.FleteCaja = txtNumLotes.Text
        addendaDTO.BodegaDestino = cmbBodegaDestinoCoppel.SelectedValue
        addendaDTO.GlnVendedor = txtGLNVendedorDS.Text
        addendaDTO.NumProveedor = getPartyIdentification(cmbComplementoFactura.SelectedValue)
    ...
    Private Sub chkDescuentoAplicado_CheckedChanged(sender As Object, e As EventArgs) Handles chkDescuentoAplicado.CheckedChanged
        If chkDescuentoAplicado.Checked Then
            Dim obj As New DataControl
            obj.RunSQLQuery("EXEC pCFD @cmd=48, @cfdid=" & Session("CFD"))
            Call itemsList_NeedData()
            Call CargaTotales()
            chkDescuentoAplicado.Enabled = False
        End If
    ...
    Private Function getPartyIdentification(ByVal complementoid) As String
        Dim Obj As New DataControl
        Return Obj.RunSQLScalarQueryString("EXEC pCFD @cmd=49, @tipo_addenda=" & complementoid)
    End Function
#End Region
    ...
    Private Sub CrearNodoComplemento(ByVal Nodo As XmlNode)
        ...
        urlcomplemento = 2
        ComercioExterior = CrearNodo("cce11:ComercioExterior")
        ...
        If txtTipoCambioAddenda.Text > 0 Then
            ComercioExterior.SetAttribute("TipoCambioUSD", Format(CDbl(txtTipoCambioAddenda.Text), "#.#00000"))
        End If
        ...
        If ds.Tables(0).Rows.Count > 0 Then
            DomicilioEmisor = CrearNodo("cce11:Domicilio")
        ...
        Receptor = CrearNodo("cce11:Receptor")
        Receptor.SetAttribute("NumRegIdTrib", NumRegIdTribx)
        ds = New DataSet
        ds = ObjData.FillDataSet("EXEC pMisClientes @cmd=2, @clienteid='" & cmbCliente.SelectedValue.ToString & "'")

        If ds.Tables(0).Rows.Count > 0 Then
            DomicilioReceptor = CrearNodo("cce11:Domicilio")
            If ds.Tables(0).Rows.Item(0)("fac_calle").ToString <> "" Then
        ...
        If ds.Tables(0).Rows.Count > 0 Then
            ...
            Mercancias = CrearNodo("cce11:Mercancias")

            For Each row As DataRow In ds.Tables(0).Rows

                Mercancia = CrearNodo("cce11:Mercancia")
    ...
    'Private Sub cmbComplementoFactura_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cmbComplementoFactura.SelectedIndexChanged
    '    panelComercioExterior.Visible = False
    '    gridResults.MasterTableView.GetColumn("NoIdentificacion").Visible = False
    '    gridResults.MasterTableView.GetColumn("FraccArancelaria").Visible = False
    '    gridResults.MasterTableView.GetColumn("CanAduana").Visible = False
    '    gridResults.MasterTableView.GetColumn("UnidadAduana").Visible = False
    '    gridResults.MasterTableView.GetColumn("UnitarioAduana").Visible = False
    '    gridResults.MasterTableView.GetColumn("ValorDolares").Visible = False

    '    'If cmbComplementoFactura.SelectedValue = 1 Then
    '    '    panelComplementoDetallista.Visible = True
    '    '    panelAddenda.Visible = False
    ...
    '    'ElseIf cmbComplementoFactura.SelectedValue = 2 Or cmbComplementoFactura.SelectedValue = 3 Then
    '    '    panelAddenda.Visible = True
    '    '    panelComplementoDetallista.Visible = False
    '    '    If cmbComplementoFactura.SelectedValue = 2 Then
    '    '        lblCantidadLotes.Visible = False
    '    '        txtCantidadLotes.Visible = False
    '    '        txtNumeroEmisorDS.Text = "10004185"
    '    '        txtGLNCompradorDS.Text = "GCD170101GS6"
    '    '        txtGLNCompradorDS.Enabled = True
    '    '        gridResults.MasterTableView.GetColumn("talla").Visible = False
    '    '        gridResults.MasterTableView.GetColumn("cajas").Visible = False
    '    '        gridResults.MasterTableView.GetColumn("piezasporcaja").Visible = False
    '    '    Else
    '    '        lblCantidadLotes.Text = "Cantidad Lotes:"
    '    '        lblCantidadLotes.Visible = True
    '    '        txtCantidadLotes.Visible = True
    '    '        txtNumeroEmisorDS.Text = "36765"
    '    '        txtGLNCompradorDS.Text = ""
    '    '        txtGLNCompradorDS.Enabled = False
    ...
    '    '    End If
    '    'ElseIf cmbComplementoFactura.SelectedValue = 5 Then
    '    '    panelAddenda.Visible = True
    '    '    panelComplementoDetallista.Visible = False
    '    '    lblCantidadLotes.Visible = True
    '    '    txtCantidadLotes.Visible = True
    '    '    txtCantidadLotes.Text = "AME0807219Q4"
    '    '    txtNumeroEmisorDS.Text = "69061"
    '    '    txtBodegaReceptora.Text = "30006"
    '    '    txtGLNCompradorDS.Text = ""
    '    '    txtGLNCompradorDS.Enabled = False
    '    '    gridResults.MasterTableView.GetColumn("talla").Visible = True
    '    '    gridResults.MasterTableView.GetColumn("cajas").Visible = True
    '    '    gridResults.MasterTableView.GetColumn("piezasporcaja").Visible = True
    '    '    lblCantidadLotes.Text = "Flete Caja:"
    '    ' Else
    '    If cmbComplementoFactura.SelectedValue = 6 Then
    '        panelComercioExterior.Visible = True
    '        gridResults.MasterTableView.GetColumn("NoIdentificacion").Visible = True
    '        gridResults.MasterTableView.GetColumn("FraccArancelaria").Visible = True
    '        gridResults.MasterTableView.GetColumn("CanAduana").Visible = True
